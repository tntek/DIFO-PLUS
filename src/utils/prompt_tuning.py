...
def test_time_tuning(model, inputs,pesu_label, optimizer, cfg):
    for j in range(cfg.DIFOPLUS.TTA_STEPS):
...
def main_worker(cfg,confi_imag,confi_dis):
    ...
    model = get_coop(cfg.DIFOPLUS.ARCH, cfg.SETTING.DATASET, int(cfg.GPU_ID), cfg.DIFOPLUS.N_CTX, cfg.DIFOPLUS.CTX_INIT)
    model = model.cuda()

    if cfg.DIFOPLUS.LOAD is not None:
        print("loading prompt")
        pretrained_ctx = torch.load(cfg.DIFOPLUS.LOAD)['ctx']
        assert pretrained_ctx.size()[0] == cfg.DIFOPLUS.N_CTX
        with torch.no_grad():
    ...
    set_id = 'sfuda'
    model.reset_classnames(classnames, cfg.DIFOPLUS.ARCH)
