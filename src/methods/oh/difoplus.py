...
def train_target(cfg):
    clip_model, preprocess,_ = clip.load(cfg.DIFOPLUS.ARCH)
    clip_model.float()
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and cfg.DIFOPLUS.CLS_PAR > 0:
            netF.eval()
            netB.eval()
            netC.eval()
        ...
        if cfg.DIFOPLUS.CLS_PAR > 0:
            ...
            classifier_loss *= cfg.DIFOPLUS.CLS_PAR
        else:
            classifier_loss = torch.tensor(0.0).cuda()

        hh = clip_all_output[tar_idx]
        iic_loss = IID_losses.IID_loss(softmax_out[indices_greater], hh[indices_greater])
        classifier_loss = classifier_loss + cfg.DIFOPLUS.IIC_PAR * iic_loss + cfg.DIFOPLUS.LENT_PAR * loss_ent

        msoftmax = softmax_out.mean(dim=0)

        gentropy_loss = torch.sum(-msoftmax * torch.log(msoftmax + cfg.DIFOPLUS.EPSILON))
        classifier_loss = classifier_loss - cfg.DIFOPLUS.GENT_PAR * gentropy_loss
        with torch.no_grad():
...
def clip_pre_text(cfg):
    ...
    prompt_prefix = cfg.DIFOPLUS.CTX_INIT.replace("_"," ")
